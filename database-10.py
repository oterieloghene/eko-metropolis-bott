...
def init_db() -> None:
    ...
    with _lock:
        ...
        #
        # role_name is unused (NULL) when access = "public".
        # ----------------------------------------------------
...
def create_sub_location(
    ...
    parent) or "role" (opens only for role_name holders).
...
def reset_database() -> int:
    ...
    NOT reset by this function, deliberately:
        ...
        `businesses`, and `business_accounts` — registered
        locations (including businesses), their attached
        sub-locations, registered government/office current
        accounts, and registered businesses with their standalone
        (never-swept-to-Central-Bank) balances must all survive a
        database reset. There is no DELETE for any of these tables
