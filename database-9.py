...
def reset_database() -> int:
    ...
    NOT reset by this function, deliberately:
        `locations`, `sub_locations`, and `current_accounts` —
        registered locations (including businesses), their
        attached sub-locations, and registered government/office
        current accounts must all survive a database reset. There
        is no DELETE for any of these tables anywhere in this
        function.
