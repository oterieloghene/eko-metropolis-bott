...
# Reuse location_admin.py's own "cat-business" -> Discord category
# name mapping instead of a second hardcoded string here. The two
# used to disagree in casing ("Business & Commerce" vs.
# "BUSINESS & COMMERCE"), and _get_or_create_category() matches by
# exact name — so !business-registration was creating a brand-new
# category every time instead of reusing the real one.
BUSINESS_CATEGORY_NAME = CATEGORY_DISCORD_NAMES["cat-business"]
...
class BusinessAdminCog(commands.Cog):
    ...
    async def business_registration(
        ...
        *,
        ...
        if created_location:
            # Merge into the road graph immediately — otherwise
            # the owner can't !walk/!drive/!taxi/!dispatch here
            # ("Unknown destination") until the bot restarts.
