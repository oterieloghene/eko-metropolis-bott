...
# Same 25/75 split as repair.py's !fixcar — the mechanic keeps
# 25% of whatever is actually paid (full or partial repair), the
# other 75% goes to the auto shop and isn't paid out to anyone,
# same as the taxi/dispatch company cuts. Kept as its own
# constant here (rather than importing repair.py's) since this
# file is intentionally self-contained from repair.py.
MECHANIC_CUT = 0.25
...
async def handle_mechanic_arrival(
    ...
    location. Runs the same repair math AND the same 25/75 payout
    split as !fixcar (repair.py is untouched — this is a parallel
    implementation, not a call into it, since !fixcar is written
    around a live ctx) and bills the OWNER, not the mechanic.
    ...
    if owner_balance < cost:
        ...
        # Mechanic still earns their cut of whatever was actually
        # spent, same as a partial !fixcar repair.
        mechanic_payout = round(owner_balance * MECHANIC_CUT)

        if mechanic_payout > 0:

            mechanic_row = database.get_player(mechanic.id)

            if mechanic_row is not None:

                database.update_player(
                    mechanic.id,
                    balance=mechanic_row["balance"] + mechanic_payout,
                )
        ...
        return (
            ...
            f"\U0001f4b0 {mechanic.mention}'s cut: \u20a6{mechanic_payout:,}"
        )
