...
class LocationAdminCog(commands.Cog):
    ...
    async def location_registration(
        ...
        *,
        ...
        # Explicit baseline: visible to everyone, but nobody can
        # chat until they actually arrive (permissions.set_write_access
        # grants send_messages per-member on arrival). Set explicitly
        # rather than left to inherit from the category — categories
        # the bot auto-creates via _get_or_create_category() have no
        # pre-existing Send Messages deny the way the hand-configured
        # static categories do, so without this, anyone could chat
        # here immediately regardless of location.
        await channel.set_permissions(
            ctx.guild.default_role,
            view_channel=True,
            send_messages=False,
            reason="Eko Bot: visible to all, write access only on arrival"
        )
        ...
        # Merge the new location into the road graph immediately —
        # without this it exists as a channel/DB row but !walk/
        # !drive/!taxi/!dispatch would still call it an "unknown
        # destination" until the bot restarts.
        routing.add_location_edge(
            code,
            zone,
            distance
        )
    ...
    async def create_sub_location(
        ...
        *,
        ...
        # IMPORTANT: without this, the bot has no explicit
        # permission overwrite of its OWN in this brand-new
        # channel. That's harmless for `public` sub-locations, but
        # for a `role`-gated one (like this) @everyone — including
        # the bot itself — was just denied view_channel above, and
        # Discord will not let the bot grant a player send_messages
        # here until the bot has that permission itself. Without
        # this call, that failure happens silently (caught and only
        # logged to console) and the room just looks permanently
        # "locked" no matter how many times a qualifying player
        # arrives. !location-registration already does this for
        # its own channel; sub-locations need it too.
        await permissions.ensure_bot_channel_permissions(ctx.guild)
