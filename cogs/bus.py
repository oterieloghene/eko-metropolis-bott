...
class BusCog(commands.Cog):
    ...
    async def purchase_bus(
        ...
        starting_location = self._route_start(
            route
        )
...
# ========================================================
    ...
    async def _run_bus(
        ...
        try:
            ...
            # ------------------------------------------------
            # RESET STOP INDEX FOR A FRESH RUN
            #
            # A bus always begins a new dispatch run sitting
            # at its route's starting stop, so stop_index must
            # start at 0 each time — otherwise a bus can only
            # ever complete one trip before it gets stuck.
            # ------------------------------------------------
