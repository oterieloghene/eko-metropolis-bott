...
# Command hints shown by the tour guide, keyed by an area's "kind"
# — every shop area shares the same three commands, every event
# area shares the same two, so this only needs to exist once
# rather than being repeated per area.
_KIND_COMMAND_HINTS = {
    "shop": (
        "`!mall`, `!spa`, or `!fastfood` to check out the mall, "
        "the private spa, or grab some local food"
    ),
    "event": (
        "`!compete` to enter a paid event, or `!try` for a free "
        "practice run"
    ),
}


def _guide_welcome(member: discord.Member, area_code: str) -> str:
    """The tour guide's in-thread arrival message — a local guide
    per area, greeting the player and pointing them at whatever
    commands actually work in this area's kind of thread."""

    cfg = AREAS[area_code]
    guide = cfg["guide"]
    hints = _KIND_COMMAND_HINTS[cfg["kind"]]

    return (
        f"{member.mention} \U0001f44b I'm **{guide}**, your tour "
        f"guide! Welcome to **{_area_label(area_code)}**.\n"
        f"Use {hints}."
    )
...
async def enter_area(guild: discord.Guild, member: discord.Member, area_code: str) -> str:
    ...
    try:
        await new_thread.send(_guide_welcome(member, area_code))
    except discord.HTTPException:
        pass
