...
def discharge_patient(patient_id: int) -> None:
    """
    Remove a patient from every ambulance-side tracking structure,
    wherever they currently sit. Called by cogs.walk.recover_player
    on EVERY recovery path (on-site !resuscitate, the collapse
    timeout auto-teleport, and an ambulance's own hospital
    drop-off) so a patient who's already back on their feet never
    lingers as a phantom "loaded" patient the next time a medic
    checks their ambulance -- e.g. someone !loadpatient'd them,
    then a different medic !resuscitate'd them on-site before the
    ambulance ever departed.
    """

    for patients in _loaded_patients.values():
        if patient_id in patients:
            patients.remove(patient_id)

    for run in _active_runs.values():
        patient_ids = run.get("patient_ids")
        if patient_ids and patient_id in patient_ids:
...
class AmbulanceCog(commands.Cog):
    ...
    async def _complete_run(
        ...
        for patient_id in patient_ids:
            ...
            if not database.is_unconscious(patient_id):
                continue
