...
def _apply_hop_decay(user_id: int, distance_km: float) -> dict:
    ...
    from cogs.weather import get_stat_drain_multiplier
    from cogs.daynight import get_stat_drain_multiplier as get_night_stat_drain_multiplier
    weather_multiplier = get_stat_drain_multiplier() * get_night_stat_drain_multiplier()

    deltas = {
        stat_name: -(rate * distance_km * weather_multiplier)
...
async def recover_player(
    ...
    from cogs.ambulance import discharge_patient
    discharge_patient(member.id)
