...
# ================================================================
# PHASE 5 — CASH REGISTER SETTLEMENT (shared by !pay and !transfer)
# ================================================================
#
...
# the lookups used below.
# ================================================================

async def _post_register_receipt(
    guild: discord.Guild,
    payer_name: str,
    business,
    register,
...
class BankingCog(commands.Cog):
    ...
    async def transfer(
        ...
        *,
        ...
        if org is not None:
            ...
            await self._post_org_receipt(ctx, org, amount, narration)
            return
        ...
        if register is not None:
            ...
            if settle_ok:
                ...
                posted = await _post_register_receipt(
                ...
                await self._post_business_receipt(ctx, business, amount, narration)

        else:
            await self._post_business_receipt(ctx, business, amount, narration)

    # ============================================================
    # PHASE 4 — business transfer receipt
    # ============================================================

    async def _post_business_receipt(
        self,
        ctx: commands.Context,
        business,
        amount: int,
        ...
        """
        Posts a lightweight receipt to the business's receipt
        channel (set via !create-business-account). Best-effort,
        same reasoning as _post_org_receipt above: the money has
        ...
        receipt_embed.add_field(name="From", value=ctx.author.display_name, inline=True)
    ...
    # ============================================================
    # PHASE 3 — org transfer receipt + cbe-log audit posting
    # ============================================================

    async def _post_org_receipt(
        self,
        ctx: commands.Context,
        org,
        amount: int,
        ...
        """
        Posts the open, unsigned receipt to the current account's
        linked channel (sender name + amount only — no sender
        balance, per spec), and a matching audit line to #cbe-log
        if that channel exists. Both are best-effort: the money
        has already moved by the time this runs, so a missing/
        deleted channel here only means a missing paper trail, not
        a failed payment.
        """
    ...
    async def view_balances(self, ctx: commands.Context):
        ...
        for row in business_accounts:
            ...
            owner = ctx.guild.get_member(int(row["owner_id"]))
            owner_label = owner.display_name if owner else f"User {row['owner_id']}"

            business_lines.append(
                f"**{row['name']}** (`{row['code']}`, owner: {owner_label}) "
                f"— ₦{row['balance']:,}"
            )
        ...
        header += "\n\n**🏪 Business Accounts**\n" + (
            "\n".join(business_lines) if business_lines else "_None yet._"
        )

        header += "\n\n**👤 Player Balances**"
        header += (
            "\n\n\u2139\ufe0f Current accounts (Phase 3) hold no balance "
            "of their own — every payment into one sweeps straight into "
            "the Central Bank of Eko balance above. Business balances "
            "above are standalone and never swept into it."
        )
