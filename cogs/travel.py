...
class TravelCog(commands.Cog):
    ...
    async def drive(
        ...
        *,
        ...
        #
        # If the driver has none queued, this is a completely
        # normal, unchanged, solo !drive trip.
        # --------------------------------------------------------

        carpool_stops = carpool.take_confirmed_stops(
            ctx.author.id
        )
        ...
        if stop_markers:
            ...
            stops_text = "\n**Passenger Stops:** " + ", ".join(
                _name(marker["destination"])
