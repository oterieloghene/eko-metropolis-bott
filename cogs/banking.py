...
Commands:
    ...
    !transfer <@player | org-code> <amount> [narration...]
        ...
        linked channel. Business recipients aren't wired up yet —
        that lands with !create-business-account (Phase 4).
    ...
    !view-balances
        ...
        Central Bank / Treasury balances. Current accounts (Phase
        3) never carry their own balance — see database.py — so
        there's nothing extra to list for them here; the Central
        Bank total already reflects every payment swept into one.
        Business balances will be added here in Phase 4.
    ...
    !adjust <@player|central_bank|treasury> <amount> [narration...]
        ...
        Business accounts will become valid targets once Phase 4
        exists.
"""
        ...
        *,
        ...
        if org is None:
            await ctx.send(
                ...
                "current account. Business account transfers "
                "aren't available yet (Phase 4)."
            )
            return
        ...
        try:
            ...
            if lowered in ("central_bank", "cbe", "centralbank"):
                ...
                await ctx.send(
                    ...
                    "Current accounts (Phase 3) hold no balance and "
                    "can never be adjusted directly; business account "
                    "adjustment isn't available yet (Phase 4)."
                )
                return
