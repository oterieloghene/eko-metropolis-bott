...
import database
from cogs import banking as banking_cog
...
# Discord hard-caps a Select to 25 options. Organizations/businesses
# beyond this many simply won't show up in the phone's dropdowns —
# !transfer <code> from a text channel still works for any of them
# regardless of this cap.
MAX_TRANSFER_SELECT_OPTIONS = 25
...
class OrgTransferModal(discord.ui.Modal, title="Pay an Organization"):
    ...
    def __init__(self, bot: commands.Bot, org_code: str):
        super().__init__()
        self.bot = bot
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Re-fetch rather than trusting whatever the dropdown saw —
        # the account could have been removed between opening the
        # menu and submitting this modal.
        org = database.get_current_account(self.org_code)

        if org is None:
            await interaction.followup.send(
                "\u26d4 That organization is no longer registered.",
                ephemeral=True
            )
            return

        ok, reason = database.current_account_transfer(
            interaction.user.id, self.org_code, amount
        )

        reasons = {
            "no_sender_account": "\u26d4 You don't have a bank account.",
            "no_such_account": "\u26d4 That organization is no longer registered.",
            "invalid_amount": "\u26d4 Enter a positive amount.",
            "insufficient_funds": "\u26d4 You don't have enough money.",
        }
        ...
        # Same shared helper !transfer uses (cogs/banking.py) — so
        # the org's channel + #cbe-log get the exact same receipt
        # and audit line regardless of whether this came from the
        # phone or the typed command.
        posted = await banking_cog.post_org_receipt(
            interaction.guild, interaction.user.display_name, org, amount
        )

        message = (
            f"\u2705 Sent \u20a6{amount:,} to {org['name']}.\n"
            f"\U0001f3e6 New Bank Balance: \u20a6{player['balance']:,}"
        )

        if not posted:
            message += (
                f"\n\u26a0\ufe0f {org['name']}'s receipt channel no "
                f"longer exists — no receipt could be posted."
            )

        await interaction.followup.send(message, ephemeral=True)


class BusinessTransferModal(discord.ui.Modal, title="Pay a Business"):
    ...
    def __init__(self, bot: commands.Bot, business_code: str):
        super().__init__()
        self.bot = bot
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Re-fetch rather than trusting whatever the dropdown saw —
        # the business (or its account) could have changed between
        # opening the menu and submitting this modal.
        business = database.get_business(self.business_code)

        if business is None or database.get_business_account(self.business_code) is None:
            await interaction.followup.send(
                "\u26d4 That business no longer has an open account.",
                ephemeral=True
            )
            return

        # Phase 5 — same open-tab check !transfer does: if the
        # sender has an open !buy tab with this business, the
        # amount must match it exactly.
        register = database.get_open_register(self.business_code, interaction.user.id)
        ...
        ok, reason = database.business_account_transfer(
            interaction.user.id, self.business_code, amount
        )

        reasons = {
            "no_sender_account": "\u26d4 You don't have a bank account.",
            "no_such_account": "\u26d4 That business no longer has an open account.",
            "invalid_amount": "\u26d4 Enter a positive amount.",
            "insufficient_funds": "\u26d4 You don't have enough money.",
        }
        ...
        message = (
            f"\u2705 Sent \u20a6{amount:,} to {business['name']}.\n"
            f"\U0001f3e6 New Bank Balance: \u20a6{player['balance']:,}"
        )

        # Same shared helpers !transfer uses (cogs/banking.py) — an
        # open tab gets the itemized receipt and is marked paid,
        # same as a phone payment settling one from !buy; otherwise
        # it's the plain receipt. Either way the business's receipt
        # channel gets the exact same paper trail as the typed
        # command.
        if register is not None:
            ...
            if settle_ok:
                ...
                posted = await banking_cog.post_register_receipt(
                    interaction.guild, interaction.user.display_name,
                    business, register
                )

                message += f"\nTab at **{business['name']}** marked paid."

                if not posted:
                    message += " (No receipt channel could be reached.)"

            else:
                message += (
                    "\n\u26a0\ufe0f The tab couldn't be automatically marked "
                    "paid (it may have changed) — the owner should sort "
                    "it out manually."
                )

                posted = await banking_cog.post_business_receipt(
                    interaction.guild, interaction.user.display_name,
                    business, amount
                )
                ...
                if not posted:
                    message += (
                        f"\n\u26a0\ufe0f {business['name']}'s receipt "
                        f"channel no longer exists — no receipt could "
                        f"be posted."
                    )
            ...
            if not posted:
                message += (
                    f"\n\u26a0\ufe0f {business['name']}'s receipt channel "
                    f"no longer exists — no receipt could be posted."
                )
...
class TransferSelectView(_BaseView):
    ...
    async def back(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            ...
            view=TransferTypeView(self.bot, self.owner_id)
        )


class OrgTransferSelect(discord.ui.Select):
    """
    Discord's UserSelect (used by TransferUserSelect above) can
    only list real Discord members — it has no way to surface a
    Phase 3 current account as a selectable target. So this is a
    plain Select instead, populated straight from
    database.all_current_accounts() (already exists — no new data
    plumbing needed).
    """

    def __init__(self, bot: commands.Bot):

        accounts = database.all_current_accounts()

        options = [
            discord.SelectOption(
                label=account["name"][:100],
                description=f"Code: {account['code']}"[:100],
                value=account["code"]
            )
            for account in accounts[:MAX_TRANSFER_SELECT_OPTIONS]
        ] or [
            discord.SelectOption(label="No organizations registered", value="__none__")
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if code == "__none__":
            await interaction.response.send_message(
                "No organizations are registered yet.", ephemeral=True
            )
            return

        org = database.get_current_account(code)

        if org is None:
            await interaction.response.send_message(
                "That organization is no longer registered.", ephemeral=True
            )
            return

        await interaction.response.send_modal(OrgTransferModal(self.bot, code))


class OrgTransferSelectView(_BaseView):

    def __init__(self, bot: commands.Bot, owner_id: int):
        super().__init__(bot, owner_id)
        self.add_item(OrgTransferSelect(bot))
...
class BusinessTransferSelect(discord.ui.Select):
    """
    Same reasoning as OrgTransferSelect above, but sourced from
    database.all_business_accounts() — every business that has an
    OPEN account (!create-business-account), not just every
    registered business.
    """

    def __init__(self, bot: commands.Bot):

        accounts = database.all_business_accounts()
        ...
        options = [
            ...
            discord.SelectOption(label="No businesses with an open account", value="__none__")
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if code == "__none__":
            await interaction.response.send_message(
                "No businesses have an open account yet.", ephemeral=True
            )
            return

        if database.get_business_account(code) is None:
            await interaction.response.send_message(
                "That business no longer has an open account.", ephemeral=True
            )
            return

        await interaction.response.send_modal(BusinessTransferModal(self.bot, code))


class BusinessTransferSelectView(_BaseView):

    def __init__(self, bot: commands.Bot, owner_id: int):
        super().__init__(bot, owner_id)
        self.add_item(BusinessTransferSelect(bot))
...
class TransferTypeView(_BaseView):
    """
    Landing menu for the Transfer button — a player, an
    organization (Phase 3 current account), or a business (Phase 4
    account) are three differently-sourced pickers under the hood
    (see OrgTransferSelect/BusinessTransferSelect above), so this
    screen exists to route to the right one before any picker is
    shown.
    """

    @discord.ui.button(label="Pay a Player", style=discord.ButtonStyle.success, emoji="\U0001f464")
    async def pay_player(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            embed=_phone_embed("Bank App \u2192 Transfer"),
            view=TransferSelectView(self.bot, self.owner_id)
        )

    @discord.ui.button(label="Pay an Organization", style=discord.ButtonStyle.primary, emoji="\U0001f3db\ufe0f")
    async def pay_org(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            embed=_phone_embed("Bank App \u2192 Transfer"),
            view=OrgTransferSelectView(self.bot, self.owner_id)
        )

    @discord.ui.button(label="Pay a Business", style=discord.ButtonStyle.primary, emoji="\U0001f3ea")
    async def pay_business(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            embed=_phone_embed("Bank App \u2192 Transfer"),
            view=BusinessTransferSelectView(self.bot, self.owner_id)
        )
