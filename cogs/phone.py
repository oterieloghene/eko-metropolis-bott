...
class HotelBookModal(discord.ui.Modal, title="Book a Hotel Room"):

    tier = discord.ui.TextInput(
        label="Tier: standard or luxury",
        placeholder="standard"
    )

    guest = discord.ui.TextInput(
        label="Guest user ID (luxury only, optional)",
        placeholder="leave blank for no guest",
        required=False
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        guest_member = None
        raw_guest = str(self.guest.value).strip()

        if raw_guest:
            guest_id = raw_guest.strip("<@!>")
            try:
                guest_member = interaction.guild.get_member(int(guest_id))
            except ValueError:
                guest_member = None

            if guest_member is None:
                await interaction.followup.send(
                    "\u26d4 Couldn't find that guest in this server.", ephemeral=True
                )
                return
...
class HotelView(_BaseView):
    ...
    @discord.ui.button(label="Book Room", style=discord.ButtonStyle.primary, emoji="\U0001f3e8")
    async def book(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(HotelBookModal(self.bot))
...
class MainMenuView(_BaseView):
    ...
    async def dispatch(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            embed=_phone_embed("Dispatch — Coming Soon"), view=ComingSoonView(self.bot, self.owner_id)
        )

    @discord.ui.button(label="Wallet", style=discord.ButtonStyle.success, emoji="\U0001f4b1", row=1)
    async def wallet(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            embed=_phone_embed("Wallet"), view=WalletView(self.bot, self.owner_id)
        )
